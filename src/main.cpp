...
// Copyright (c) 2013 Bonus developers
...
const string strMessageMagic = "Bonus Signed Message:\n";
...
//global values
static const int64_t nTargetSpacing = 2 * 60;   //2 minute blocks

//default values
static const int64_t nTargetTimespan = 120 * 60; //120 minute intervals (60 blocks)
...
//specialized values for launch
static const int64_t nTargetTimespan_0 = 12 * 60; //~12 minute intervals (6 blocks)
static const int64_t nInterval_0 = nTargetTimespan_0 / nTargetSpacing;
...
unsigned int static GetNextWorkRequired_V0(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    //set the difficulty for the first interval to set the base level
    if ((pindexLast->nHeight+1) > 0 && pindexLast->nHeight+1 < nInterval_0)
    {
        return 0x1e017798;
    }

    // Only change once per interval
    if ((pindexLast->nHeight+1) % nInterval_0 != 0)
    {
        return pindexLast->nBits;
    }

    // Go back by what we want to be 20 minutes worth of blocks
    const CBlockIndex* pindexFirst = pindexLast;
    for (int i = 0; pindexFirst && i < nInterval_0-1; i++)
    ...
    if (nActualTimespan < nTargetTimespan_0/2.5)
        nActualTimespan = nTargetTimespan_0/2.5;
    if (nActualTimespan > nTargetTimespan_0*2.5)
        nActualTimespan = nTargetTimespan_0*2.5;
    ...
    if(fDebug) {
        ...
        printf("nTargetTimespan = %"PRI64d"    nActualTimespan = %"PRI64d"\n", nTargetTimespan_0, nActualTimespan);
...
unsigned int static KimotoGravityWell(const CBlockIndex* pindexLast, const CBlockHeader *pblock, uint64 TargetBlocksSpacingSeconds, uint64 PastBlocksMin, uint64 PastBlocksMax) {
    ...
    if(fDebug) {
        ...
        printf("PastRateActualSeconds = %"PRI64d"\n", PastRateActualSeconds);
        printf("PastRateTargetSeconds = %"PRI64d"\n", PastRateTargetSeconds);
        printf("PastBlocksMass = %"PRI64d"\n", PastBlocksMass);
        printf("PastDifficultyAveragePrev = %08x  %s\n", PastDifficultyAveragePrev.GetCompact(), PastDifficultyAveragePrev.getuint256().ToString().c_str());
...
{
    static const int64 BlocksTargetSpacing = 120; // 2 minutes
	unsigned int TimeDaySeconds = 60 * 60 * 24;
    int64 PastSecondsMin = TimeDaySeconds * 0.1;
    int64 PastSecondsMax = TimeDaySeconds * 2.5;
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    if (pindexLast->nHeight+1 >= 2880) {
        DiffMode = 2; //KGW
    } else if (pindexLast->nHeight+1 >= 120) {
        DiffMode = 1; //BTCs
    }
...
bool FindBlockPos(CValidationState &state, CDiskBlockPos &pos, unsigned int nAddSize, unsigned int nHeight, uint64 nTime, bool fKnown = false)
{
    ...
    if (fKnown) {
        ...
        while (infoLastBlockFile.nSize + nAddSize >= MAX_BLOCKFILE_SIZE) {
            if (fDebug) { printf("Leaving block file %i: %s\n", nLastBlockFile, infoLastBlockFile.ToString().c_str()); }
            FlushBlockFile(true);
            nLastBlockFile++;
            infoLastBlockFile.SetNull();
    ...
    if (!fKnown) {
        ...
        if (nNewChunks > nOldChunks) {
            if (CheckDiskSpace(nNewChunks * BLOCKFILE_CHUNK_SIZE - pos.nPos)) {
                ...
                if (file) {
                    if (fDebug) { printf("Pre-allocating up to position 0x%x in blk%05u.dat\n", nNewChunks * BLOCKFILE_CHUNK_SIZE, pos.nFile); }
...
bool FindUndoPos(CValidationState &state, int nFile, CDiskBlockPos &pos, unsigned int nAddSize)
{
    ...
    if (nNewChunks > nOldChunks) {
        if (CheckDiskSpace(nNewChunks * UNDOFILE_CHUNK_SIZE - pos.nPos)) {
            ...
            if (file) {
                if (fDebug) { printf("Pre-allocating up to position 0x%x in rev%05u.dat\n", nNewChunks * UNDOFILE_CHUNK_SIZE, pos.nFile); }
...
bool static LoadBlockIndexDB()
{
    ...
    if (fDebug) { printf("LoadBlockIndexDB(): last block file = %i\n", nLastBlockFile); }
    if (pblocktree->ReadBlockFileInfo(nLastBlockFile, infoLastBlockFile))
        if (fDebug) { printf("LoadBlockIndexDB(): last block file info: %s\n", infoLastBlockFile.ToString().c_str()); }
    ...
    if (fDebug) { printf("LoadBlockIndexDB(): transaction index %s\n", fTxIndex ? "enabled" : "disabled"); }
...
bool VerifyDB(int nCheckLevel, int nCheckDepth)
{
   ...
   if (fDebug) { printf("Verifying last %i blocks at level %i\n", nCheckDepth, nCheckLevel); }
...
bool LoadExternalBlockFile(FILE* fileIn, CDiskBlockPos *dbp)
{
    ...
    try {
        ...
        while (blkdat.good() && !blkdat.eof()) {
            ...
            try {
                ...
                if (fDebug) { printf("%s() : Deserialize or I/O error caught during load\n", __PRETTY_FUNCTION__); }
            }
        }
        fclose(fileIn);
    } catch(std::runtime_error &e) {
    ...
    if (nLoaded > 0)
        if (fDebug) { printf("Loaded %i blocks from external file in %"PRI64d"ms\n", nLoaded, GetTimeMillis() - nStart); }
    return nLoaded > 0;
}
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
    ...
    else if (strCommand == "getheaders")
    {
        ...
        int nLimit = 2000;
        if (fDebug) { printf("getheaders %d to %s\n", (pindex ? pindex->nHeight : -1), hashStop.ToString().c_str()); }
        for (; pindex; pindex = pindex->GetNextInMainChain())
        {
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BonusMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BonusMiner : ProcessBlock, block not accepted");
    }
...
void static BitcoinMiner(CWallet *pwallet)
{
    ...
    try { loop {
        ...
        if (fDebug) {
            printf("Running BonusMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
