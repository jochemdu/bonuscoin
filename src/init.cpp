...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: bonus.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: bonusd.pid)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Make sure only a single Bonus process is using the data directory.
    boost::filesystem::path pathLockFile = GetDataDir() / ".lock";
    FILE* file = fopen(pathLockFile.string().c_str(), "a"); // empty lock file; created if it doesn't exist.
    if (file) fclose(file);
    static boost::interprocess::file_lock lock(pathLockFile.string().c_str());
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Bonus is probably already running."), strDataDir.c_str()));
    ...
    if (fDaemon)
        fprintf(stdout, "Bonus server starting\n");
    ...
    // requested to kill bonus-qt during the last operation. If so, exit.
    ...
    if (nLoadWalletRet != DB_LOAD_OK)
    {
        ...
        else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
        {
            ...
            strErrors << _("Error loading wallet.dat: Wallet requires newer version of Bonus") << "\n";
        else if (nLoadWalletRet == DB_NEED_REWRITE)
        {
            strErrors << _("Wallet needed to be rewritten: restart Bonus to complete") << "\n";
